...
import { Ban, CheckCircle2, ChevronDown, Download, Eye, Plus, Search, Trash2, UserRoundCheck } from "lucide-react";
...
function VipDialog({ open, onOpenChange }: { open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  function selectTradesman(userId: string) {
    ...
    setVipBySkill(tradesmanProfile?.vipBySkill || "");
    setTradesmanListOpen(false);
    setTradesmanSearch("");
  }
...
export default function UsersPage() {
  ...
  function actions(user: User) { return <div className="flex items-center justify-end gap-2"><button disabled={block.isPending} title={user.isBlocked ? "Activate user" : "Block user"} aria-label={user.isBlocked ? "Activate user" : "Block user"} className={cn("text-red-600 disabled:opacity-50", user.isBlocked && "text-amber-600")} onClick={() => block.mutate(user._id)}>{user.isBlocked ? <UserRoundCheck size={17} /> : <Ban size={17} />}</button>{user.tradesmanProfile?.verificationStatus === "pending" && <button disabled={verify.isPending} title="Verify tradesman" aria-label="Verify tradesman" className="text-green-600 disabled:opacity-50" onClick={() => verify.mutate({ id: user.tradesmanProfile!._id })}><CheckCircle2 size={17} /></button>}<button title="View member" aria-label="View member" className="text-blue-500" onClick={() => setSelected(user)}><Eye size={17} /></button><button title="Delete member" aria-label="Delete member" className="text-red-500" onClick={() => { if (confirm(`Delete ${nameOf(user)}?`)) remove.mutate(user._id); }}><Trash2 size={17} /></button></div>; }
          ...
          {selected.tradesmanProfile && <>
            <section className="rounded-md border border-black/10 p-4">
              ...
              <dl className="grid gap-x-6 gap-y-4 sm:grid-cols-2">
                ...
                <DetailItem label="VIP skill">{selected.tradesmanProfile.vipBySkill || "Not provided"}</DetailItem>
